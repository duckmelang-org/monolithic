...
public class ReviewReportQueryStrategy implements ReportQueryStrategy<ReviewReport> {
    private final ReviewReportRepository repository;

    @Override
    public Page<ReviewReport> queryByDateOrder(ReportStatus status, Pageable page) {
        return repository.findByStatusAndPageWithDate(status, page);

    }

    @Override
    public Page<ReviewReport> queryByCountOrder(ReportStatus status, Pageable page) {
