...
import umc.duckmelang.domain.report.repository.PostReportRepository;
...
public class PostReportQueryStrategy implements ReportQueryStrategy<PostReport> {
    private final PostReportRepository repository;

    @Override
    public Page<PostReport> queryByDateOrder(ReportStatus status, Pageable page) {
        return repository.findByStatusAndPageWithDate(status, page);
    }

    @Override
    public Page<PostReport> queryByCountOrder(ReportStatus status, Pageable page) {
